...
import RegisterEmailStep from '@/features/auth/components/RegisterEmailStep';
...
export default function RegisterPage() {
  ...
  const handleEmailContinue = (userEmail: string) => {
    setEmail(userEmail);
    setStep('details');
  };
        ...
        <div className="w-full max-w-xl space-y-12">
          <div className="space-y-6 text-center">
            <h2 className="text-4xl lg:text-5xl font-bold text-[#252627] leading-tight">
              Conceal. Create.<br />Communicate.
            </h2>
          </div>
          
          {/* Register Forms */}
          {step === 'email' ? (
            <RegisterEmailStep onContinue={handleEmailContinue} />
          ) : (
            <RegisterDetailsForm email={email} onBack={handleBack} />
          )}
        </div>
      </div>
